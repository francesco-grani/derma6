"""HTML export generator for the Skincare Plan download."""
...
def _desc(ingredient: str) -> str:
    ...
    if k in STEP_DESCRIPTIONS:
        return STEP_DESCRIPTIONS[k]
    for key, val in STEP_DESCRIPTIONS.items():
...
def generate_export_html(username: str) -> str:
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .avatar {
        ...
        font-size: 22px; font-weight: 700; color: #1C2520;
        flex-shrink: 0;
    }
    ...
    .index a { font-size: 15px; font-weight: 500; color: #E0E8E0; }
    .index a:hover { color: #7A9B7D; }

    /* Section titles */
    .section { margin-bottom: 56px; }
    .section-title {
        font-size: 20px; font-weight: 700; color: #E0E8E0;
        padding-bottom: 10px;
        border-bottom: 2px solid #4B5A4C;
        margin-bottom: 24px;
    }
    ...
    .profile-label {
        font-size: 11px; font-weight: 600; color: #9EAD9E;
    ...
    .routine-name {
        font-size: 17px; font-weight: 600; color: #E0E8E0;
        margin-bottom: 14px;
    }
    .step-card {
        background: #fff;
    ...
    .step-category {
        color: #9EAD9E; font-size: 11px; letter-spacing: .08em;
    }
    ...
    /* Footer */
    .footer {
        text-align: center; color: #9EAD9E; font-size: 12px;
        margin-top: 60px; padding-top: 24px;
        border-top: 1px solid #4B5A4C;
    }

    @media print {
        body { background: #fff; color: #1C2520; }
        .header, .index, .profile-card { background: #f4f6f4; }
        .bubble.ai { box-shadow: none; border: 1px solid #ddd; }
    }
    """
    ...
    # ------------------------------------------------------------------ Index
    index_items = ["Profile", "My Routines", "My Chat"]
    index_html = "".join(
        f'<li><a href="#{item.lower().replace(" ", "-")}">{_e(item)}</a></li>'
    ...
    def _field(label: str, value_html: str) -> str:
        ...
        <div class="profile-card">
          <div class="profile-label">{_e(label)}</div>
          <div class="profile-value">{value_html}</div>
        </div>"""

    if profile:
        skin_type_val = _e(profile.skin_type.capitalize()) if profile.skin_type else "—"
        concerns_val = (
            "".join(f'<span class="badge">{_e(c)}</span>' for c in profile.skin_concerns)
            if profile.skin_concerns else "—"
        )
        shaving_val = (
            "Yes" if profile.has_shaving_routine is True
            else ("No" if profile.has_shaving_routine is False else "—")
        )
        flags_val = (
            "".join(f'<span class="badge badge-medical">{_e(f)}</span>' for f in profile.medical_flags)
            if profile.medical_flags else "None"
        )
        onboarding_val = "✅ Complete" if profile.onboarding_complete else "⏳ In progress"

        profile_html = f"""
        <div class="profile-grid">
          {_field("Skin Type", skin_type_val)}
          {_field("Skin Concerns", concerns_val)}
          {_field("Shaving Routine", shaving_val)}
          {_field("Medical Flags", flags_val)}
          {_field("Onboarding", onboarding_val)}
        </div>
        """
    else:
    ...
    if routines:
        ...
        for routine in routines:
            ...
            for i, step in enumerate(routine.steps, 1):
                ...
                cat = _e(CATEGORY_LABELS.get(ing.strip().lower(), "STEP"))
                desc = _e(_desc(ing))
                steps_html += f"""
                <div class="step-card">
                  ...
                  <div class="step-body">
                    <div class="step-header">
                      <span class="step-ingredient">{display}</span>
                      <span class="step-category">{cat}</span>
                    </div>
                    <div class="step-desc">{desc}</div>
                  </div>
                </div>"""
            routine_blocks.append(f"""
            <div class="routine-block">
              <div class="routine-name">{_e(routine.name)}</div>
              {steps_html}
            </div>""")
    ...
    routines_section = f"""
    <div class="section" id="my-routines">
    ...
    if messages:
        ...
        for msg in messages:
            ...
            bubbles.append(f"""
            <div>
              <div class="role-label {role}">{_e(label)}</div>
    ...
    chat_section = f"""
    <div class="section" id="my-chat">
      <div class="section-title">My Chat</div>
      {chat_html}
    </div>
    """
