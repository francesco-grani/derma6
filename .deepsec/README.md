...
Currently configured project: `fgrani-AE.CAP.AFA.1.1` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/fgrani-AE.CAP.AFA.1.1/SETUP.md` to fill in
   `data/fgrani-AE.CAP.AFA.1.1/INFO.md`.

## Daily commands

This workspace authenticates via `OPENROUTER_API_KEY` in `.env.local`, not
`AI_GATEWAY_API_KEY` — `pi` is the only backend that can route through
OpenRouter, and model / `--ai-api-key-env` aren't configurable in
`deepsec.config.ts`, so use the `pnpm run` scripts below instead of calling
`deepsec process`/`revalidate` bare:

```bash
pnpm deepsec scan
pnpm run process:openrouter    --concurrency 5
pnpm run revalidate:openrouter --concurrency 5              # cuts FP rate
...
`--project-id fgrani-AE.CAP.AFA.1.1` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage — via OpenRouter on
GPT-5.4, real-world cost has run ≈$0.03–$4 per file depending on how many
cross-file leads the agent chases (heavier files cost more; see run history
in `data/fgrani-AE.CAP.AFA.1.1/files/*.json`). Run state goes to
`data/fgrani-AE.CAP.AFA.1.1/`.
